...
可以看出，`db.onlyDoc.find({title: /spark/i}).sort({year: 1})`是由于year字段上索引的建立，避免了全表扫描。查询总时间从

**31866显著降低到67**。

> executionStats.executionTimeMillis:67
>
> executionStats.totalKeysExamined:4524
>
> executionStats.totalDocsExamined:4524
>
> totalReturn:1575
>
...
由于在year建立的索引，排序操作通过执行索引扫描（IXSCAN）在很短时间内就完成了。主要的时间都花费在排序结果寻找对应数据上。

###结论

可以通过在排序关键字建立索引显著增加带排序操作的查询的查询速度
