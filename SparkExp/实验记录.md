...
我们使用一个正则匹配`A`作为查询关键字，这个查询将返回2316690个文档。

查询命令：`db.onlyDoc.find({title: /A/}).sort({year: 1})`

显示查询、执行计划命令：`db.onlyDoc.explain("allPlansExecution").find({title: /A/}).sort({year: 1})`
...
[
  {
    "executionStats": {
      ...
      "nReturned": 1,
      "executionTimeMillis": 119444,
      "totalKeysExamined": 0,
      "totalDocsExamined": 5146977,
      "executionStages": {
        "stage": "SORT",
        "nReturned": 1,
        "executionTimeMillisEstimate": 45610,
        "works": 5146982,
        "advanced": 1,
        "needTime": 5146980,
        "needYield": 0,
        ...
        "inputStage": {
          ...
          "nReturned": 2316690,
          "executionTimeMillisEstimate": 45328,
          "works": 5146980,
          "advanced": 2316690,
          "needTime": 2830289,
          "needYield": 0,
          ...
          "inputStage": {
            ...
            "nReturned": 2316690,
            "executionTimeMillisEstimate": 44399,
            "works": 5146979,
            "advanced": 2316690,
            "needTime": 2830288,
            "needYield": 0,
...
查询命令：`db.onlyDoc.find({title: /A/})`

显示查询、执行计划命令：`db.onlyDoc.explain("allPlansExecution").find({title: /A/})`
...
`db.onlyDoc.find({title: /A/}).sort({year: 1})`是一个会返回大量结果的的查询语句，根据执行计划显示的内容

> executionStats.executionTimeMillis:176474
>
> executionStats.totalDocsExamined:5146977

该查询语句由于执行了全表扫描，总共执行时间接近3分钟。执行了如下三个过程

```mermaid
graph LR;
  SORT-->SORT_KEY_GENERATOR
  SORT_KEY_GENERATOR-->COLLSCAN
```
...
pie
    title 各阶段执行耗时估计比例
    "SORT" : 97465
    "SORT_KEY_GENERATOR" : 96216
    "COLLSCAN" : 96216
```

但是`db.onlyDoc.find({title: /A/})`的执行却花费了极少时间。按照执行阶段的好事估计值来看，该查询应该也需要花费大约1分钟的时间。但是实际上花费的时间却非常少
...
由于只需要获取较少数量的结果（由LIMIT阶段决定这个结果的数量），表扫描（COLLSCAN）阶段只需要花费很少的时间，搜集到满足LIMIT阶段所规定数量的文档就可以完成查询任务。

###结论

对未建立索引的返回结果避免不必要的sort操作，以免延长查询时间。
